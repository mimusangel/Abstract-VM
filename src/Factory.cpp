...
{
    return (new Operand<char>(INT8, std::atoll(value.c_str()), 0, std::numeric_limits<char>::min(), std::numeric_limits<char>::max()));
}

IOperand const *Factory::createInt16(std::string const &value) const
{
    return (new Operand<int>(INT16, std::atoll(value.c_str()), 1, std::numeric_limits<int>::min(), std::numeric_limits<int>::max()));
}

IOperand const *Factory::createInt32(std::string const &value) const
{
    return (new Operand<long>(INT32, std::atoll(value.c_str()), 2, std::numeric_limits<long>::min(), std::numeric_limits<long>::max()));
}
